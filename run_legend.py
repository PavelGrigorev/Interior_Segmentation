...
def run(input_path, output_path, model_path, model_type="dpt_hybrid", optimize=True):
    ...
    for ind, img_name in enumerate(img_names):
        ...
        out_dict = dict(zip(unique, counts/prediction.size * 100))
        out_dict = {k: v for k, v in out_dict.items() if v > 1.}
        for k in out_dict:
          out_dict.update({ k : [round(out_dict[k],2)] + classes[k-1] })
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_path", default="pics4segm", help="folder with input images"
    )
